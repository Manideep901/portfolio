...
export default function ThemeSelector() {
  ...
  useEffect(() => {
    const systemThema = window.matchMedia("(prefers-color-scheme: dark)");
    console.log("dhdhgfdhfgadhfd", systemThema?.matches);

    if (systemThema?.matches) {
      setTheme("dark");
    } else {
      setTheme("light");
    }
  ...
  const toggleThemeIcons = useCallback(() => {
  ...
  return (
    <div className="relative">
      <div className="theme-selector">
        {openIcons ? (
          <div className="relative z-0 inline-grid grid-cols-3 gap-0.5 rounded-full  p-0.75 theme">
            {themeIcons.map((icon, index) => (
              <span
                key={index}
                className="cursor-pointer rounded-full p-1.5 *:size-7 "
          ...
          <div className="relative z-0 inline-grid  rounded-full  p-0.75  theme selected">
            <span
              className="cursor-pointer rounded-full p-1.5 *:size-7 "
              onClick={toggleThemeIcons}
            >
